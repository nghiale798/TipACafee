...
class LanguageController extends Controller
{
    ...
    public function updateAllTranslations($id)
{
    try {
        ...
        if (!file_exists($filePath)) {
            return response()->json(['success' => false, 'message' => 'File ngôn ngữ không tồn tại!']);
        }

        // Đọc nội dung file JSON hiện tại
        $translations = json_decode(file_get_contents($filePath), true);

        // Lấy dữ liệu từ ngôn ngữ mặc định (ví dụ: tiếng Anh)
        $defaultLang = resource_path('lang/en.json');
        if (!file_exists($defaultLang)) {
            return response()->json(['success' => false, 'message' => 'File ngôn ngữ mặc định không tồn tại!']);
        }

        $defaultTranslations = json_decode(file_get_contents($defaultLang), true);

        // Cập nhật các key chưa có trong file JSON
