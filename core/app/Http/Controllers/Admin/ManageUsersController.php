...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $totalSupporterCount = Donation::success()->where('user_id', $user->id)->where('member_id', 0)->count();
        $totalMemberCount    = Membership::active()->where('user_id', $user->id)->count();

        $totalSupporter = Donation::success()->where('user_id', $user->id)->where('member_id', 0)->sum('amount');
        $totalMember    = Donation::success()->where('user_id', $user->id)->where('supporter_id', 0)->sum('amount');

        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        return view('admin.users.detail', compact('pageTitle', 'user', 'totalDeposit', 'totalWithdrawals', 'totalTransaction', 'countries',  'totalSupporterCount', 'totalMemberCount', 'totalSupporter', 'totalMember'));
    }
