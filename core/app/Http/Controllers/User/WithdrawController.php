...
class WithdrawController extends Controller
{
    ...
    public function withdrawLog(Request $request)
    {
        ...
        $query                  = Withdrawal::where('user_id', auth()->id())->where('status', '!=', Status::PAYMENT_INITIATE);
        $countWithdraw          = (clone $query)->count();
        $lastThirtyDaysWithdraw = (clone $query)->where('created_at', '>', now()->subDays(30)->endOfDay())->sum('amount');
        $totalWithdraw          = (clone $query)->sum('amount');

        if ($request->search) {
            $query = $query->where('trx', $request->search);
        }
        $withdraws = $query->with('method')->orderBy('id', 'desc')->paginate(getPaginate());
        return view($this->activeTemplate . 'user.withdraw.log', compact('pageTitle', 'withdraws', 'countWithdraw', 'lastThirtyDaysWithdraw', 'totalWithdraw'));
    }
}
