                        ...
                        <table class="table table--light">
                            ...
                            <tbody>
                                @forelse($galleryImages as $gallery)
                                    <tr>
                                        <td>
                                            <span class="fw-bold"> {{ @$gallery->user->fullname }}</span>
                                            <br>
                                            <a href="{{ route('admin.users.detail', $gallery->user_id) }}"> {{ @$gallery->user->username }}</a>
                                        </td>
                                        <td>
                                            <a class="fw-bold" href="{{ route('gallery.view', [$gallery->user->profile_link, $gallery->slug, $gallery->id]) }}" target="_blank"> {{ strLimit($gallery->title, 30) }} </a>
                                        </td>
                                        <td>
                                            @if ($gallery->status == Status::VISIBLE_PUBLIC)
                                        ...
                                        <td>
                                            {{ showDateTime($gallery->created_at) }}
                                            <br>
                                            {{ diffForHumans($gallery->created_at) }}
                                        </td>
                                        <td>
                                            <div class="button--group">
                                                <a class="btn btn-sm btn-outline--primary ms-1" href="{{ route('admin.gallery.details', $gallery->id) }}">
                                                    <i class="las la-desktop"></i> @lang('Details')
                                                </a>

                                                <a class="btn btn-sm btn-outline--info ms-1" href="{{ route('admin.gallery.comment', $gallery->id) }}">
