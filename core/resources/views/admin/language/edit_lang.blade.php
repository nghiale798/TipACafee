...
<div class="col-md-5 mt-md-0 mt-3">
    <button type="button" class="btn btn-sm btn-outline--success float-end updateAllBtn">
        <i class="la la-sync"></i> @lang('Cập nhật tất cả')
    </button>
</div>
@endpush
...
<script>
    (function ($) {
        ...
        $(document).on('click', '.updateAllBtn', function () {
            if (!confirm('@lang("Bạn có chắc muốn cập nhật tất cả các câu dịch?")')) return;

            $.ajax({
                type: "POST",
                url: "{{ route('admin.language.update.all', $lang->id) }}",
                data: {
                    _token: "{{ csrf_token() }}"
                },
                beforeSend: function () {
                    notify('info', '@lang("Đang cập nhật tất cả...")');
                },
                success: function (response) {
                    if (response.success) {
                        notify('success', '@lang("Tất cả câu dịch đã được cập nhật!")');
                        setTimeout(function () {
                ...
                error: function () {
                    notify('error', '@lang("Lỗi kết nối, vui lòng thử lại!")');
                }
            });
        });
