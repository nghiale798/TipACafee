<form id="editGalleryForm" action="" method="POST" enctype="multipart/form-data">
    <div class="modal fade add-img-modal custom--modal" id="edit-img-modal" aria-labelledby="addimg-modal" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header d-flex justify-content-between">
                    <h4 class="modal-title">@lang('Update Gallery Photo')</h4>
                    <button data-bs-dismiss="modal" type="button" aria-label="Close">
                        <span class="icon fs-20"><i class=" fa
                            fa-times"></i></span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="gallery-modal-item  gallery-img mb-3">
                        <img class="profilePicPreview" src="{{ avatar(null) }}" title="@lang('Image resized into') {{ getFileSize('gallery') }}@lang('px')" alt="gallery-img">
                        <label for="edit-gallery-img">
                            <input id="edit-gallery-img" name="image" type="file" accept="image/*">
                    ...
                    <div class="gallery-modal-item">
                        ...
                        <div class="form-group">
                            <label class="privet-message form--check" for="notify-followers-edit">
                                <span class="custom--check">
                                    <input class="form-check-input" id="notify-followers-edit" name="notify_followers" type="checkbox">
                                </span>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(".edit-photo-btn").on('click', function() {
                ...
                // Set the image src
                $('.profilePicPreview').attr('src', photoSrc);

                // Populate form fields - assuming resource contains appropriate attributes
                $('input[name="id"]').val(resource.id);
                $('input[name="title"]').val(resource.title);
                $('textarea[name="content"]').val(resource.content);
                $('select[name="status"]').val(resource.status).change();
                $('select[name="visible"]').val(resource.visible);

                // Show the modal
                $("#edit-img-modal").modal('show');
            });

            $("#edit-gallery-img").on('change', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            notify('success', `@lang('Gallery photo update successfully')`);
                        } else {
