...
(function ($, undef) {
  ...
  function legacyWebkitGradient(origin, colorList) {
    ...
    $.each(colorList, function (i, val) {
      stops.push(
        "color-stop(" + parseFloat(val.stop) / 100 + ", " + val.color + ")"
      );
    });
        ...
        if (item !== self.active) {
          switch (item) {
            case "strip":
              ...
              value =
                controlOpts.strip === "h"
                  ? self._scale[controlOpts.strip] - color[controlOpts.strip]
            ...
            case "square":
              ...
              cssObj = {
                left:
                  (color[controlOpts.horiz] / self._scale[controlOpts.horiz]) *
                  dimensions.w,
                top:
                  dimensions.h -
                  (color[controlOpts.vert] / self._scale[controlOpts.vert]) *
                    dimensions.h,
              };
...
(function (exports, undef) {
  ...
  for (var key in shortProps) {
    if (shortProps.hasOwnProperty(key)) Color.fn[key] = Color.fn._partial(key);
  }
